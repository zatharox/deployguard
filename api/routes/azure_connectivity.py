...
async def test_azure_connection(
    request: AzureTestRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    client = AzureDevOpsClient(
        organization=request.organization, project=request.project, pat=request.pat
    )
    ...
    logger.info(
        "azure_connection_test_completed",
        user_id=current_user.id,
        status=status,
...
async def get_webhook_setup_guide(current_user: User = Depends(get_current_user)):
    """
