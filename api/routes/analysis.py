...
async def run_demo_scenario(
    ...
    if scenario not in {"low", "medium", "high", "critical"}:
        raise HTTPException(
            status_code=400,
            detail="scenario must be one of: low, medium, high, critical",
        )
    ...
    if reset:
        ...
        db.query(PipelineHistory).filter(
            PipelineHistory.tenant_id == tenant.id
        ).delete()
        db.commit()
    ...
    if scenario == "low":
        ...
        file_histories = [
            FileHistory(
                tenant_id=tenant.id,
                file_path="/auth/auth_service.py",
                change_count=40,
                failure_count=14,
            ),
            FileHistory(
                tenant_id=tenant.id,
                file_path="/payment/payment_api.py",
                change_count=35,
                failure_count=8,
            ),
            FileHistory(
                tenant_id=tenant.id,
                file_path="/database/repository.py",
                change_count=32,
                failure_count=9,
            ),
        ]
    ...
    now = datetime.utcnow()
    max_run_id = (
        db.query(func.max(PipelineHistory.run_id))
        .filter(PipelineHistory.tenant_id == tenant.id)
        .scalar()
        or 500000
    )
...
async def get_unstable_files(
    ...
    **Use this to:**
    ...
    unstable_files = [f for f in files if f.failure_rate >= min_failure_rate]
...
async def get_summary_stats(
    ...
    **Perfect for:** Dashboards, monitoring, and reporting
    ...
    total_analyses = (
        db.query(PRAnalysis).filter(PRAnalysis.tenant_id == tenant.id).count()
    )
    ...
    total_files = (
        db.query(FileHistory).filter(FileHistory.tenant_id == tenant.id).count()
    )
