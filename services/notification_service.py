...
class NotificationService:
    ...
    async def _send_email_alert(
        ...
        emoji = {"high": "🔴", "critical": "⛔", "medium": "🟡"}.get(
            result.risk_level, "🟢"
        )

        subject = (
            f"{emoji} High Risk Deployment Alert - PR #{pr_id} in {repository_name}"
        )
    ...
    async def _send_slack_alert(
        ...
        }.get(result.risk_level, ":white_check_mark:")

        color = {"high": "#ef4444", "critical": "#dc2626", "medium": "#f59e0b"}.get(
            result.risk_level, "#10b981"
        )
